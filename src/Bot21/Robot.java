...
public abstract class Robot {
    ...
    public Robot(RobotController rc, int id) throws GameActionException {
        ...
        this.justDied = false;
        this.microAttacker = new MicroAttacker(rc);
    ...
    void initTurn() throws GameActionException {
        ...
        if (this.justSpawned && RobotPlayer.bitAt(this.byteZero, 12-(id%3))) {
             ...
             if (rc.isSpawned() && rc.getLocation().distanceSquaredTo(curDest) > 50)
    ...
    public void senseFlags() throws GameActionException {
        if (id != 0 || rc.getRoundNum() != 1)
            return;

        MapLocation[] locs = rc.senseBroadcastFlagLocations();
        for (int i=0; i<locs.length; i++) {
            rc.writeSharedArray(10+i, MapHelper.poseEncoder(locs[i]));
        }
    ...
    class AttackTarget{
        ...
        AttackTarget(RobotInfo r){
            health = r.getHealth();
            mloc = r.getLocation();
            if (r.hasFlag) hasFlag = true;
        }
    }

    public boolean shouldPickUpFlag(Direction dir, Direction goalDir) {
        return dir == Direction.CENTER || dir == goalDir || dir == goalDir.rotateLeft() || dir == goalDir.rotateRight();
    }
}
